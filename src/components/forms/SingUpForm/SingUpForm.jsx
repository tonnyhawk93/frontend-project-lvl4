...
function SingUpForm() {
  ...
  const onSubmit = useCallback(async ({ username, password }, { setSubmitting, setErrors }) => {
    ...
    try {
      ...
      setErrors({ signup: 'Пользователь с таким ником уже существует' });
      setSubmitting(false);
    }
  }, [logIn]);
      ...
      <form onSubmit={formik.handleSubmit}>
        <div className="form-floating mb-3">
          <input
            ...
            className={cn('form-control', { 'is-invalid': formik.touched.username && (formik.errors.username || formik.errors.signup) })}
            value={formik.values.username}
            onChange={formik.handleChange}
            required
          />
          <label htmlFor="username">{t('singUpForm.usernamePlaceholder')}</label>
          {(formik.errors.username || formik.errors.signup) && formik.touched.username && (
            <div className="invalid-feedback">{formik.errors.username || formik.errors.signup}</div>
          )}
        </div>
        <div className="form-floating mb-3">
          <input
            ...
            required
          />
          <label htmlFor="password">{t('singUpForm.passwordPlaceholder')}</label>
        ...
        <div className="form-floating mb-3">
          <input
            ...
            required
          />
          <label htmlFor="confirmationPassword">{t('singUpForm.passwordConfirmationPlaceholder')}</label>
